...
(define (build-image-table p at)
  ...
  (when
      (or (not (valid-bitpix? at))
          (not (eq? 0 (hash-ref at "PCOUNT" #f)))
          (not (eq? 1 (hash-ref at "GCOUNT" #f)))
          (not (and (exact-integer? (hash-ref at "NAXIS"))
                    (<= (hash-ref at "NAXIS") 999))))
    (error 'IMAGE-extension "wrong attribute value (BITPIX, GCOUNT, PCOUNT, NAXIS)"))
  ; 准备初始化的解析条件
  (let* ([bitpix (cast (hash-ref at "BITPIX") Integer)]
         [naxis_c (cast (hash-ref at "NAXIS") Integer)]
         [naxis_ax
    ...
    (define (recur-decode-axis ax b)
      (if (= 1 (length ax))
          ...
          (let ([bvec (split-bytes-vec b bitpix)])
            (if (= (vector-length bvec) (car ax)) bvec
          ...
          (let ([subvec
            ...
            (for/vector ([_ (in-range (car ax))])
              (recur-decode-axis subvec)))))
    ;   解析开始
    (image-table (list->vector naxis_ax)
                 (recur-decode-axis naxis_ax data_bytes))))